	...
	switch (state)
	{
		...
		case CANNONS_STATE_ATTACKING_TOP:
		{
			...
			/*if (listBullet.size() <= 4)
			{
				...
				listBullet.push_back(bullet2);
			}*/
			listBullet.push_back(bullet);
			listBullet.push_back(bullet2);
			//listBullet.erase(listBullet.begin() + 1);
			break;
		}
		case CANNONS_STATE_ATTACKING_LEFT:
		{
			//Attack
			Bullet* bullet = new BigNavigatedEnemyBullet(x, y + CANNONS_BBOX_HEIGHT / 2 - 3.0f, EntityType::TAG_CANNONS, -1, 0);
			Bullet* bullet2 = new BigNavigatedEnemyBullet(x + CANNONS_BBOX_WIDTH, y + CANNONS_BBOX_HEIGHT / 2 - 3.0f, EntityType::TAG_CANNONS, 1, 0);
			/*if (listBullet.size() <= 4)
			{
				listBullet.push_back(bullet);
				listBullet.push_back(bullet2);
			}*/
			//listBullet.erase(listBullet.begin() + 0, listBullet.begin() + 1);
			listBullet.push_back(bullet);
			listBullet.push_back(bullet2);
			break;
		}
